...
char usage[] = 	"spicmd read/write eeprom_address data(if write)\n"\
	...
	"  spicmd vtss p4\n"\
	"  spicmd vtss novlan\n"\
    "  spicmd intermcu write\n"\
    "  spicmd intermcu read\n"\
...
int main(int argc, char *argv[])
{
    ...
    if (!strncmp(argv[1], "intermcu", 8)) {
        SPI_INTERMCU intermcu;
        unsigned char buf[4] = {0x55, 0xaa, 0x00, 0xFF};
		...
		if (!strncmp(argv[2], "write", 5)) {
            ...
            printf("SPICMD: SPI write done\n");
		} else if (!strncmp(argv[2], "read", 4)) {
            intermcu.size = 4;
            intermcu.buf = buf;
			ioctl(fd, RT2880_SPI_INTERMCU_READ, &intermcu);
            printf("SPICMD: SPI read done 0x%x, 0x%x, 0x%x, 0x%x\n", buf[0], buf[1], buf[2], buf[3]);
		}
		close(fd);
		return 0;
    }
