...
PRIVATE char * form_data_encode (const char * baseurl, HTAssocList * formdata)
{
    if (formdata) {
	BOOL first = YES;
	int cnt = HTList_count((HTList *) formdata);
	...
	while (cnt > 0) {
	    pres = (HTAssoc *) HTList_objectAt((HTList *) formdata, --cnt);
        if (strcmp(HTAssoc_name(pres), "BOUNDARY") == 0) {
	        HTChunk_putc(fullurl, '-');
	        HTChunk_putc(fullurl, '-');
	        HTChunk_puts(fullurl, HTAssoc_value(pres));
	        HTChunk_putc(fullurl, '\r');
	        HTChunk_putc(fullurl, '\n');
        } else if (strcmp(HTAssoc_name(pres), "FILE") == 0) {
            HTChunk_puts(fullurl, "Content-Disposition: form-data; name=\"file\"; filename=\"upload/1/sleep/ting\"");
            HTChunk_putc(fullurl, '\r');
            HTChunk_putc(fullurl, '\n');
            HTChunk_puts(fullurl, "Content-Type: application/json\r\n");
            HTChunk_puts(fullurl, HTAssoc_value(pres));
            HTChunk_putc(fullurl, '\r');
            HTChunk_putc(fullurl, '\n');
        } else {
            HTChunk_puts(fullurl, "Content-Disposition: form-data; name=\"");
            HTChunk_puts(fullurl, HTAssoc_name(pres));
            HTChunk_puts(fullurl, "\"\r\n\r\n");
            HTChunk_puts(fullurl, HTAssoc_value(pres));
            HTChunk_putc(fullurl, '\r');
            HTChunk_putc(fullurl, '\n');
        }
					    ...
					    HTRequest *		request)
{
	...
	return HTPostFormAnchor(formdata, anchor, request, 0);
    }
    return NULL;
}
                      ...
                      int           typeForm)
{
	...
	char * form_encoded;
    if (typeForm == 0) {
        form_encoded = form_url_encode(NULL, formdata);
    } else {
        form_encoded = form_data_encode(NULL, formdata);
    }
	if (form_encoded) {
	    ...
	    */
        ...
        if (typeForm == 0) {
	        HTAnchor_setFormat(postanchor, WWW_FORM_CHARSET);
        } else {
	        HTAnchor_setFormat(postanchor, WWW_MULTIPART_FORM);
        }
                      ...
                      int           typeForm)
{
	...
	HTStream * target = HTStreamToChunk(request, &chunk, 0);
	HTRequest_setOutputStream(request, target);
	if (HTPostFormAnchor(formdata, anchor, request, typeForm) != NULL)
	    return chunk;
	else {
	    HTChunk_delete(chunk);
	    return NULL;
