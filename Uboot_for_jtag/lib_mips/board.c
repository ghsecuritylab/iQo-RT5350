...
/* Adjust CPU Freq from 60Mhz to 600Mhz(or CPLL freq stored from EE) */
	...
	*(volatile u_long *)(RALINK_SYSCTL_BASE + 0x0B10) = cpu_to_le32(value);	
	 ...
	 */	
	debug ("relocate_code AAA Pointer at: %08lx\n", addr);
	relocate_code (addr_sp, id, /*TEXT_BASE*/ addr);	
#else
	debug ("relocate_code BBB Pointer at: %08lx\n", addr);
...
/*config bootdelay via environment parameter: bootdelay */
	...
	if(BootType == '3') {
		...
		case '4':
            ...
            /*
            // Disable Ethernet Port && PHY
            mii_mgr_write(0, 0, 0x3900);
            printf ("port 1 done\n");
            ...
            printf ("port 4 done\n");
            *(unsigned long *)(0xb0110090) = 0x0F807F7F;
            printf ("port done\n");
            */
