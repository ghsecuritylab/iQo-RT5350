...
#define RALINK_SPI_UPGRADE_CHECK    1
...
#else
	ra_outl(RT2880_SPI0_CFG_REG, SPICFG_MSBFIRST | SPICFG_TXCLKEDGE_FALLING | SPICFG_SPICLK_DIV128 | SPICFG_SPICLKPOL);
#endif

	udelay(1000);
    ra_outl(RT2880_SPI_ARB_REG, 0);
	udelay(1000);
...
unsigned long raspi_init(void)
{
    /*
    test GPIO
	ra_outl(RT2880_GPIOMODE_REG, 0x00004000);
    ra_outl((RT2880_PRGIO_ADDR + 0x74), 0x0000001C);
    while (1) {
        ra_outl((RT2880_PRGIO_ADDR + 0x84), 0x0000001C);
        udelay(100000);
    }
    */
    spic_init();
